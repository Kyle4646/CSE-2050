...
class CustomSet:
    ...
    def _rehash(self, new_buckets):
        ...
        newbuckets = [[] for i in range(new_buckets)]
        ...
        for bucket in self._L:
            for item in bucket:
                new_bucket_idx = hash(item) % new_buckets
